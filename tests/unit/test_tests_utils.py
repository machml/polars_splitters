...
def test_get_uniformly_distributed_combinations_of_two_bools(n, k):
    if n % (2**k) == 0:
        treatment_outcome_pairs = get_uniformly_distributed_combinations_of_two_bools(
            n=n,
            k=k,
        )
        ...
        check.equal(
            Counter(zip(treatments, outcomes, strict=False)),
            {(0, 0): n // 4, (1, 1): n // 4, (0, 1): n // 4, (1, 0): n // 4},
        )
