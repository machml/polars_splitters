...
class TestSplitIntoTrainEval:
    ...
    def test_output_types_e_sizes_inputting_basic_df(
        ...
        check.is_instance(df_eval, DataFrame)
        ...
        if not shuffle:
            check.is_true(df_train.shape[0] + df_eval.shape[0] == df_basic().shape[0])
            df_concat = concat([df_train, df_eval])
    ...
    def test_stratification_inputting_basic_df(self, df_basic, stratify_by):
        ...
        if stratify_by is None:
            check.equal(Counter(df_train["treatment"]), Counter({1: 6, 0: 1}))
            check.equal(Counter(df_eval["treatment"]), Counter({0: 3}))
        elif stratify_by == "treatment":
            check.equal(Counter(df_train["treatment"]), Counter({1: 4, 0: 3}))
            check.equal(Counter(df_eval["treatment"]), Counter({1: 2, 0: 1}))
        elif stratify_by == ["treatment", "outcome"]:
            check.equal(
                Counter(zip(df_train["treatment"], df_train["outcome"], strict=False)),
                Counter({(0, 0): 3, (1, 0): 2, (1, 1): 2}),
            )
            check.equal(
                Counter(zip(df_eval["treatment"], df_eval["outcome"], strict=False)),
    ...
    def test_stratification_inputting_df_ubools(self, df_ubools, stratify_by):
        ...
        eval_rel_size = 0.3
        df_train, df_eval = split_into_train_eval(
            df=df_ubools(n=n_input, seed=SEED),
            eval_rel_size=eval_rel_size,
            stratify_by=stratify_by,
            shuffle=False,
            seed=SEED,
        ...
        if stratify_by is None:
            check.equal(Counter(df_train["treatment"]), Counter({0: 132, 1: 148}))
            check.equal(Counter(df_eval["treatment"]), Counter({0: 68, 1: 52}))
        elif stratify_by == "treatment":
            check.equal(Counter(df_train["treatment"]), Counter({0: 140, 1: 140}))
            check.equal(Counter(df_eval["treatment"]), Counter({0: 60, 1: 60}))
            check.equal(
                Counter(zip(df_train["treatment"], df_train["outcome"], strict=False)),
...
class TestSplitIntoKFolds:
    ...
    def test_output_types_e_sizes_inputting_df_ubools(
        ...
        n_input = 400
        df_input = df_ubools(n=n_input, seed=SEED)
        rel_size_deviation_tolerance = 0.1
        ...
        for fold in folds:
            ...
            if not shuffle:
                check.is_true(df_train.shape[0] + df_eval.shape[0] == n_input)
                df_concat = concat([df_train, df_eval])
                n_duplicates = df_concat.is_duplicated().sum()
                check.is_true(n_duplicates == 0)
